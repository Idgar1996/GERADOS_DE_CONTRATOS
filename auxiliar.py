...
def limpar_todos_campos(*frames):

       # Função genérica para limpar todos os campos de entrada (Entry) em um frame.
    for frame in frames:
        for widget in frame.winfo_children():  # Itera sobre todos os widgets do frame
            if isinstance(widget, CTkEntry):  # Verifica se o widget é um campo de entrada
                widget.delete(0, 'end')  # Limpa o conteúdo do campo
            elif isinstance(widget, CTkComboBox):  # Verifica se o widget é um combobox
                widget.set("Selecione")  # Reseta o combobox para o valor padrão
...
def verificar_e_atualizar_senha():
    ...
    if not nome or not nova_senha or not confirma_nova_senha or \
       ...
       pergunta3 == "Selecione" or not resposta3:
        ...
        app.after(3000, lambda: resultado_recuperacao.configure(text=''))
        return
...
botao_voltar = CTkButton(
    ...
    text_color='white',
    font=('Source Sans Pro', 20, 'bold', 'underline'),
    command=lambda: [limpar_todos_campos(frame_login,frame_recuperar_senha), mostrar_frame(frame_login)]
)
botao_voltar.place(relx=0.95, rely=0.5, anchor='center')
...
botao_voltar = CTkButton(
    ...
    text_color='white',
    font=('Source Sans Pro', 20, 'bold', 'underline'),
    command=lambda: [limpar_todos_campos(frame_dados_cliente), mostrar_frame(frame_opcoes)]
)
botao_voltar.place(relx=0.95, rely=0.5, anchor='center')
